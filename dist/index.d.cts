...
interface AttestationSelection<T extends SelectableAttestation> {
     ...
     * means do not display it in the chooser AND do not let the user pick
     * it — the response would be unsignable or rejected by the verifier.
     */
    usable: boolean;
    /**
     * Why `usable` is false (or `"ok"` when usable). The only reason today
     * is `"wrong_holder"` — the attestation was issued to a different
     * holder pubkey than the wallet currently holds. Other reasons (e.g.
     * issuer-allow-list mismatch) are RP-side policy and are NOT enforced
     * here; the wallet has no way to know each RP's policy.
     */
    reason: "ok" | "wrong_holder";
    /**
