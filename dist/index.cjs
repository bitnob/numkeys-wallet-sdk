...
function selectUsableAttestations(args) {
  const { request, attestations, walletPubkey } = args;
  const candidateHash = request.scope === "phone" && request.candidate_phone_e164 ? phoneHash(request.candidate_phone_e164) : null;
  return attestations.map((a) => {
    const matchesCandidatePhone = candidateHash === null ? null : a.phoneHash === candidateHash;
    let reason;
    if (a.userPubkey !== walletPubkey) {
      reason = "wrong_holder";
    } else if (candidateHash !== null && matchesCandidatePhone === false) {
...
exports.assembleSignedResponse = assembleSignedResponse;
exports.buildErrorResponse = buildErrorResponse;
exports.buildRedirectUrl = buildRedirectUrl;
exports.bytesToB64Url = bytesToB64Url;
exports.canonicalJson = canonicalJson;
exports.decodeRpVerifyRequest = decodeRpVerifyRequest;
exports.deliverPopupResponse = deliverPopupResponse;
...
exports.selectUsableAttestations = selectUsableAttestations;
exports.validateRpVerifyRequest = validateRpVerifyRequest;
